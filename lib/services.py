...
class AppService:
    ...
    def create_task(self, name, description='',  start_date=datetime.now(),
                    end_date=None, parent_task=None,
                    group=None, assigned=None):
    ...
    def get_user_folders(self):
        return list(self.current_user.folders)  # rework later.

    def get_user_tasks(self):
        return self.session.query(
            Task).filter(Task.owner == self.current_user).all()
    ...
    def delete_task_by_id(self, id):
        self.session.query(Task).filter_by(id=id).delete()
        self.session.commit()
